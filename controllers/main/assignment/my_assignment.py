...
class GateDetection:
    ...
    def _estimate_solvepnp(self, drone_state: dict, drone_pos: np.ndarray) -> Optional[GateEstimate]:
...
class DroneFSM:
    ...
    def step(self, drone_state, camera_frame) -> Setpoint:
        if self.state == State.TAKEOFF:
            ...
            drone_orientation = np.array([drone_state['roll'], drone_state['pitch'], drone_state['yaw']])
            print(f"[TAKEOFF]  Current drone position: {drone_pos[0]:.2f} m, {drone_pos[1]:.2f} m, {drone_pos[2]:.2f} m, target: {self.TAKEOFF_Z} m")
            print(f"[TAKEOFF]  Current drone orientation: {drone_orientation[0]:.2f} rad, {drone_orientation[1]:.2f} rad, {drone_orientation[2]:.2f} rad, target: {self.TAKEOFF_YAW:.2f} rad")
    ...
    def _handle_search(self, drone_state, camera_frame) -> Setpoint:
        ...
        if estimate is not None:
            ...
            print(f"[SEARCH → FLY_TO_GATE]  Gate {self.current_gate_idx} "
                  f"detected at {self.gate_position} with yaw {estimate.yaw:.1f}° "
